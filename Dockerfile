FROM debian:jessie
MAINTAINER Nurimba <contato@nurimba.com> (https://github.com/nurimba)
...
RUN npm install -g gulp bower jest-cli

RUN echo "%sudo ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers && \
    useradd -u 5001 -G users,sudo -d /nb --shell /bin/bash -m nb && \
    echo "secret\nsecret" | passwd nb

RUN usermod -u 1000 nb
